PROJECT_NAME := hetznerdns Package
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-hetznerdns,${REPOSITORY},g' provider/go.mod; \
...
build_nodejs:: install_plugins tfgen # build the node sdk
	...
	cd sdk/nodejs/ && \
				jq ".scripts.install=.scripts.install + \" $(VERSION) --server https://api.github.com/repos/spion/pulumi-hetznerdns/releases\"" 	package.json > package.json.tmp && mv package.json.tmp package.json && \
				yarn install && \
        yarn run tsc && \
		cp -R scripts/ bin && \
